...
const showLoginToast = (title: string, message: string, variant: 'success' | 'error' | 'warning', duration: number) => {
  const titleColors = {
    success: '#166534',
    error: '#991b1b',
    warning: '#92400e',
  ...
  toast.custom((t) => (
    <div className="flex items-start gap-3 p-4 rounded-xl shadow-lg" style={{ background: '#fef3c7', border: '2px solid #f59f24' }}>
      ...
      <div className="flex-1">
        <p className="font-semibold text-sm" style={{ color: titleColors[variant] }}>{title}</p>
        <p className="text-xs mt-0.5" style={{ color: messageColors[variant] }}>{message}</p>
      </div>
...
const Connexion = () => {
  ...
  useEffect(() => {
    ...
    const redirectIfAuthenticated = async () => {
      ...
      if (cachedUser) {
        navigate(redirectTo || getDefaultRedirect(cachedUser.role), { replace: true });
        return;
      }

      const user = await AuthService.fetchCurrentUser();
      if (user) {
        navigate(redirectTo || getDefaultRedirect(user.role), { replace: true });
        return;
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.user) {
        ...
        showLoginToast("Connexion réussie !", "Bienvenue sur votre espace membre CEEAM.", 'success', 3000);
        ...
        if (isValidActivityId) {
          ...
          navigate(redirectTo || getDefaultRedirect(response.user.role));
        }
