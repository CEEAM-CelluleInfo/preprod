...
export const ProposeActivityPage: React.FC = () => {
  ...
  const validateForm = () => {
    ...
    if (!formData.contact.trim()) {
      newErrors.contact = 'L\'email de contact est requis';
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload: ActivityProposalCreateData = {
        ...
        contact_email: formData.contact.trim(),
        additional_info: details.join(' | ') || undefined,
      };

      await ActivityProposalService.submitProposal(payload, selectedImageFile);
      alert('Votre proposition a ete soumise avec succes.');
      navigate('/activites');
    } catch (error: any) {
      console.error('Erreur lors de la soumission:', error);
      alert(error?.detail || 'Erreur lors de la soumission. Veuillez reessayer.');
    } finally {
                ...
                <div className="grid grid-cols-2 gap-6 mb-6">
                  <div className="flex flex-col gap-2">
                    ...
                    <input
                      ...
                      className={`px-4 py-3 border-2 rounded-lg text-base transition-all focus:outline-none focus:border-orange-500 focus:shadow-[0_0_0_3px_rgba(249,115,22,0.1)] ${errors.contact ? 'border-red-500' : 'border-slate-200'}`}
                      required
                    />
                    {errors.contact && (
